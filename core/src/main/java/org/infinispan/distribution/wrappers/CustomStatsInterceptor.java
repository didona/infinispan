...
public final class CustomStatsInterceptor extends BaseCustomInterceptor {
   ...
   @ManagedAttribute(description = "LocalXactCommitLogGetReadVersion",
           displayName = "LocalXactCommitLogGetReadVersion")
   public final long getLocalXactCommitLogGetReadVersion() {
      return handleLong((Long) TransactionsStatisticsRegistry.getAttribute(COMMIT_LOG_GET_READ_VERSION, null));
   }

   @ManagedAttribute(description = "LocalXactCommitLogGetVersionLessThan",
           displayName = "LocalXactCommitLogGetVersionLessThan")
   public final long getLocalXactCommitLogGetVersionLessThan() {
      return handleLong((Long) TransactionsStatisticsRegistry.getAttribute(COMMIT_LOG_GET_AVAILABLE_VERSION_LESS_THAN, null));
   }

   @ManagedAttribute(description = "LocalXactCommitLogGetReadVersionCount",
           displayName = "LocalXactCommitLogGetReadVersionCount")
   public final long getLocalXactCommitLogGetReadVersionCount() {
      return handleLong((Long) TransactionsStatisticsRegistry.getAttribute(COMMIT_LOG_GET_READ_VERSION_COUNT, null));
   }

   @ManagedAttribute(description = "LocalXactCommitLogGetVersionLessThanCount",
           displayName = "LocalXactCommitLogGetVersionLessThanCount")
   public final long getLocalXactCommitLogGetVersionLessThanCount() {
      return handleLong((Long) TransactionsStatisticsRegistry.getAttribute(COMMIT_LOG_GET_AVAILABLE_VERSION_LESS_THAN_COUNT, null));
   }

   @ManagedAttribute(description = "RemoteUpdateXactTimeBetweenAckAndCommit",
   ...
   private void handleCommitCommand(TransactionStatistics transactionStatistics, long initTime, long initCpuTime, TxInvocationContext ctx) {
      ...
      //This is valid both for local and remote. The registry will populate the right container
         ...
         if (ctx.isOriginLocal()) {
            ...
            transactionStatistics.addValue(REMOTE_TIME_BETWEEN_ACK_AND_COMMIT, System.nanoTime() - ((RemoteTransactionStatistics) transactionStatistics).getValue(REMOTE_ACK_TIME));
         }
      }
   ...
   private void handlePrepareCommand(TransactionStatistics transactionStatistics, long initTime, long initCpuTime, TxInvocationContext ctx, PrepareCommand command) {
      ...
      //This is valid both for local and remote. The registry will populate the right container
         ...
         if (ctx.isOriginLocal()) {
            ...
            //If the xact is update and remote, then set this time, so that you can compute the time lasting between the ack and the final commit, if any
            //I set this time also for xacts that do not actually enter the queue, because in any case they receive the commit
            transactionStatistics.addValue(REMOTE_ACK_TIME, System.nanoTime());
         }
